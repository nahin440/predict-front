...
export default function HeroSection({ prediction }: Props) {
  ...
  return (
    <section style={{ minHeight:"100dvh", position:"relative", overflow:"hidden", display:"flex", alignItems:"center", paddingTop:100 }}>
      ...
      {/* Gold radial glow */}
      <div style={{ position:"absolute",top:"-10%",left:"50%",transform:"translateX(-50%)",width:900,height:600,borderRadius:"50%",background:"radial-gradient(ellipse,rgba(245,166,35,0.1) 0%,transparent 70%)",pointerEvents:"none",filter:"blur(60px)",zIndex:1 }} />
          ...
          <div>
            ...
            <div className="animate-fade-up d0" style={{ display:"inline-flex",alignItems:"center",gap:8,marginBottom:28 }}>
              <div className="badge" style={{ background:"rgba(0,230,118,0.06)",border:"1px solid rgba(0,230,118,0.25)",color:"var(--up)",padding:"5px 14px",borderRadius:99 }}>
                ...
                <span style={{ fontFamily:"JetBrains Mono",fontSize:10,letterSpacing:"0.1em" }}>AI SIGNALS LIVE · XAUUSD</span>
              </div>
            </div>

            {/* Sliding headline — LaunchDarkly: line1 white, line2 gold */}
            <div style={{ marginBottom:24, minHeight:"clamp(100px,12vw,165px)", overflow:"hidden" }}>
              {SLIDE_HEADLINES.map((h,i)=>(
                <div key={i} style={{
                  ...
                  <h1 style={{ fontFamily:"Syne",fontWeight:800,fontSize:"clamp(36px,5.5vw,72px)",lineHeight:1.0,letterSpacing:"-0.04em" }}>
            ...
            <p className="animate-fade-up d2" style={{ fontFamily:"Space Grotesk",fontSize:"clamp(15px,1.5vw,17px)",color:"var(--fog)",lineHeight:1.7,maxWidth:480,marginBottom:36 }}>
            ...
            <div className="animate-fade-up d3" style={{ marginBottom:40 }}>
              <div className="input-hero" style={{ maxWidth:480 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--slate)" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,12 2,6"/></svg>
                <input
                  type="email"
              ...
              <p style={{ fontFamily:"JetBrains Mono",fontSize:10,color:"var(--slate)",marginTop:10,letterSpacing:"0.05em" }}>
            ...
            {patterns && patterns.length > 0 && (
              <div className="animate-fade-up d4" style={{ overflow:"hidden",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",background:"var(--carbon)",padding:"8px 0",maxWidth:480 }}>
                <div className="ticker-inner">
                  {[...patterns,...patterns].map((p,i)=>(
                    <span key={i} style={{ fontFamily:"JetBrains Mono",fontSize:10,color:"var(--slate)",padding:"0 18px",borderRight:"1px solid rgba(255,255,255,0.05)",whiteSpace:"nowrap",letterSpacing:"0.06em" }}>
                      {p}
                    </span>
                  ))}
                </div>
          ...
          <div className="animate-fade-up d2 hide-mobile" style={{ position:"relative" }}>
            ...
            <div className="animate-float" style={{ position:"absolute",top:-20,right:20,zIndex:10,animationDelay:"0.5s" }}>
              <div className="card" style={{ padding:"10px 16px",background:"var(--graphite)",border:"1px solid rgba(245,166,35,0.2)" }}>
                <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:2 }}>Regime</div>
                <div style={{ fontFamily:"Syne",fontSize:14,fontWeight:700,color:"var(--gold)" }}>{regime||"TRENDING_BEAR"}</div>
              </div>
            </div>
            ...
            <div className="animate-float" style={{ position:"absolute",bottom:40,left:-20,zIndex:10,animationDelay:"1s" }}>
              <div className="card" style={{ padding:"10px 16px",background:"var(--graphite)",border:"1px solid rgba(0,230,118,0.2)" }}>
                <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:2 }}>Grade</div>
                <div style={{ fontFamily:"Syne",fontSize:18,fontWeight:800,color:"var(--up)" }}>
            ...
            <div className="card" style={{
              ...
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22 }}>
                <div>
                  <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:5 }}>XAUUSD · AI Signal</div>
                  <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                    <div className="live-dot" />
                    <span style={{ fontFamily:"JetBrains Mono",fontSize:10,color:"var(--up)" }}>Processing Active</span>
                  </div>
                </div>
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",marginBottom:3 }}>SPOT PRICE</div>
                  <div style={{ fontFamily:"JetBrains Mono",fontSize:28,fontWeight:500,lineHeight:1 }}>
                ...
                <>
                  <div style={{ padding:"18px 22px",borderRadius:16,border:`1px solid ${sigColor}28`,background:`${sigColor}07`,marginBottom:18 }}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                      <div>
                        <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6 }}>Signal</div>
                        <div style={{ fontFamily:"Syne",fontSize:36,fontWeight:800,color:sigColor,lineHeight:1 }}>{sigText}</div>
                        {Boolean(prediction.signal_strength) && (
                          <div style={{ fontFamily:"JetBrains Mono",fontSize:10,color:sigColor,opacity:0.7,marginTop:4 }}>
                            {String(prediction.signal_strength)}
                          </div>
                        )}
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",marginBottom:3 }}>Confidence</div>
                        <div style={{ fontFamily:"JetBrains Mono",fontSize:38,fontWeight:300,color:conf>=60?"var(--gold-bright)":"var(--ash)" }}>
                    ...
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:18 }}>
                      {Object.entries(prediction.model_votes as Record<string,number>).map(([m,v])=>(
                        <div key={m} style={{ textAlign:"center",padding:"10px 8px",borderRadius:12,background:"var(--graphite)",border:"1px solid rgba(255,255,255,0.05)" }}>
                          <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",textTransform:"uppercase",marginBottom:3 }}>{m}</div>
                          <div style={{ fontFamily:"JetBrains Mono",fontSize:18,fontWeight:500,color:v>0.5?"var(--down)":"var(--up)" }}>
                            {(v*100).toFixed(0)}%
                          </div>
                        </div>
                      ))}
                  ...
                  <div style={{ position:"relative",borderRadius:12,overflow:"hidden" }}>
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,filter:"blur(5px)",userSelect:"none",pointerEvents:"none" }}>
                      {[["SL","$4335.75"],["TP1","$4329.11"],["RR","1.5:1"]].map(([l,v])=>(
                        <div key={l} style={{ padding:"10px",borderRadius:10,background:"var(--graphite)",textAlign:"center" }}>
                          <div style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",marginBottom:2 }}>{l}</div>
                          <div style={{ fontFamily:"JetBrains Mono",fontSize:13,color:"var(--paper)" }}>{v}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,background:"rgba(8,8,9,0.55)",backdropFilter:"blur(2px)",borderRadius:12 }}>
                      <span style={{ fontFamily:"JetBrains Mono",fontSize:9,color:"var(--slate)",textTransform:"uppercase",letterSpacing:"0.1em" }}>Trader Plan Required</span>
        ...
        <div className="animate-fade-up d6" style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginTop:56,paddingTop:40,borderTop:"1px solid rgba(255,255,255,0.05)" }} id="stats-row">
          {[
            ...
            <div key={i} style={{ textAlign:"center" }}>
              <div style={{ fontFamily:"Syne",fontSize:"clamp(28px,3vw,40px)",fontWeight:800,color:"var(--gold)",lineHeight:1,marginBottom:4 }}>{s.v}</div>
              <div style={{ fontFamily:"Space Grotesk",fontSize:13,fontWeight:600,color:"var(--paper)",marginBottom:2 }}>{s.l}</div>
              <div style={{ fontFamily:"Space Grotesk",fontSize:11,color:"var(--slate)" }}>{s.s}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <style>{`
        ...
        @media(max-width:480px){ #stats-row{ grid-template-columns:1fr 1fr !important; gap:8px !important; } }
      `}</style>
    </section>
  );
}
