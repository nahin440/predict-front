...
import { getPredictionModel, isTimeframe } from "@/models/Prediction";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ tf: string }> }) {
  try {
    const { tf } = await params;
    if (!isTimeframe(tf)) {
      return NextResponse.json({ success: false, error: "Invalid timeframe. Use m15, h1, or h4." }, { status: 400 });
    }
    ...
    if (!isTrader) {
      ...
      return NextResponse.json({ success: true, data: safeData, tier: "free", timeframe: tf });
    }

    // Pro tier: everything
    if (isPro) {
      return NextResponse.json({ success: true, data: latest, tier: "pro", timeframe: tf });
    }
    ...
    return NextResponse.json({ success: true, data: traderData, tier: "trader", timeframe: tf });
  } catch (err) {
