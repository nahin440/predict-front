import { NextResponse } from 'next/server';
import { MongoClient } from 'mongodb';

const uri = "mongodb+srv://preedict:EcsyQ60wspFtrMKY@cluster0.slbhc.mongodb.net/?appName=Cluster0";
const client = new MongoClient(uri);

export async function GET() {
  try {
    await client.connect();
    const db = client.db("btc_dashboard");
    const collection = db.collection("btc_predictions");
    const latest = await collection.findOne({}, { sort: { server_time: -1 } });
    return NextResponse.json({ prediction: latest });
  } catch (error) {
    return NextResponse.json({ error: "Failed to fetch BTC latest" }, { status: 500 });
  } finally {
    await client.close();
  }
}